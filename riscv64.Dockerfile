...
RUN curl -o /scripts/qemu.deb -k -L $(apt-get download --print-uris qemu-user-static | cut -d"'" -f2); dpkg -x /scripts/qemu.deb /tmp; rm -rf /scripts/qemu.deb; cp /tmp/usr/bin/qemu-x86_64-static /usr/bin/qemu-x86_64-static; cp -Rf /tmp/usr/libexec* /usr/libexec; cp -Rf /tmp/usr/share/binfmts /usr/share/binfmts; cp -Rf /tmp/usr/bin/* /usr/bin; rm -rf /tmp/*
